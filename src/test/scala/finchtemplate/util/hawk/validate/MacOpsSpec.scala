...
final class MacOpsSpec extends Specification with ScalaCheck with SpecHelper {
  "Arrays of bytes" >> {
    "can have a SHA-256 MAC created" >> {
      ...
      MacOps.mac(creds, "".getBytes) must beEqualTo(MAC(Base64Encoded("mhiOwqitx35nIxKVmvYU3w/JmOmjGExRa0Qd/Mm+nuc=")))
      MacOps.mac(creds, "the cat in the hat".getBytes) must beEqualTo(MAC(Base64Encoded("hJpSQNabjqx/gn1E5JSIEaCgCVtTlWMcdp+62WbQumc=")))
      MacOps.mac(creds,
    ...
    "can have a SHA-512 MAC created" >> {
      ...
      MacOps.mac(creds, "".getBytes) must beEqualTo(MAC(Base64Encoded("d6TUoj/rcgNSF/Qheb3Y671dk8Ic8mH8KTWo0gkuJHrsP+fnUmrcynOh7yyRctpIrKstWha9GM7dDAwsuprwDA==")))
      MacOps.mac(creds, "the cat in the hat".getBytes) must beEqualTo(MAC(Base64Encoded("ggctPQsPjrGKbeEovn3R20dsIeMSrQzHqab/h0s/nkoemDQ+KhioX7beGplHduwr4ShZMVf+VboatpWv3d4cVQ==")))
      MacOps.mac(creds,
        ("There is no expectation that the client will adjust its system clock to match the server (in fact, this would" +
          " be a potential attack vector). Instead, the client only uses the server's time to calculate an offset used " +
          "only for communications with that particular server. The protocol rewards clients with synchronized clocks " +
